...
public class product_detailDao {
	...
	public product_detailVo selectByPdd_id(int pdd_id) {
		...
		try {
			...
			String sql = "select * from product_detail where pdd_id=?;";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, pdd_id);
			...
			vo.setPdd_stk_count(rs.getInt("pdd_stk_count"));
			}
	...
	public product_detailVo getPdd_info(int pd_id, String col_id, String sz_id) {
		product_detailVo vo = new product_detailVo();
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			conn = ConnUtil.getConnection();
			String sql = "select * from product_detail where pd_id=? and col_id=? and sz_id=?;";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, pd_id);
			ps.setString(2, col_id);
			ps.setString(3, sz_id);
	...
	public ArrayList<product_detailVo> getPdd_info(int pd_id) {
		...
		try {
			conn = ConnUtil.getConnection();
			String sql = "select * from product_detail where pd_id=?;";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, pd_id);
			rs = ps.executeQuery();
			while (rs.next()) {
