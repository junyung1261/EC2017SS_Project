package ec.company;

public class companyVo {
	private String co_name;
	private String co_ceo;
	private String co_phone;
	private String co_tel;
	private String co_fax;
	private String co_address;
	private String co_email;
	private String co_delivery;
	
	private String co_reg_num1;
	private String co_reg_num2;
	
	private String co_bank;
	private String co_account;
	
	private String co_id;
	private String co_password;
	private String co_grade;
	
	private String co_reg_date;
	private String co_exp_date;
	
	private String co_tags;  

	
	public String getCo_name() {
		return co_name;
	}

	public void setCo_name(String co_name) {
		this.co_name = co_name;
	}

	public String getCo_ceo() {
		return co_ceo;
	}

	public void setCo_ceo(String co_ceo) {
		this.co_ceo = co_ceo;
	}

	public String getCo_phone() {
		return co_phone;
	}

	public void setCo_phone(String co_phone) {
		this.co_phone = co_phone;
	}

	public String getCo_tel() {
		return co_tel;
	}

	public void setCo_tel(String co_tel) {
		this.co_tel = co_tel;
	}

	public String getCo_fax() {
		return co_fax;
	}

	public void setCo_fax(String co_fax) {
		this.co_fax = co_fax;
	}

	public String getCo_address() {
		return co_address;
	}

	public void setCo_address(String co_address) {
		this.co_address = co_address;
	}

	public String getCo_email() {
		return co_email;
	}

	public void setCo_email(String co_email) {
		this.co_email = co_email;
	}

	public String getCo_delivery() {
		return co_delivery;
	}

	public void setCo_delivery(String co_delivery) {
		this.co_delivery = co_delivery;
	}

	public String getCo_reg_num1() {
		return co_reg_num1;
	}

	public void setCo_reg_num1(String co_reg_num1) {
		this.co_reg_num1 = co_reg_num1;
	}

	public String getCo_reg_num2() {
		return co_reg_num2;
	}

	public void setCo_reg_num2(String co_reg_num2) {
		this.co_reg_num2 = co_reg_num2;
	}

	public String getCo_bank() {
		return co_bank;
	}

	public void setCo_bank(String co_bank) {
		this.co_bank = co_bank;
	}

	public String getCo_account() {
		return co_account;
	}

	public void setCo_account(String co_account) {
		this.co_account = co_account;
	}

	public String getCo_id() {
		return co_id;
	}

	public void setCo_id(String co_id) {
		this.co_id = co_id;
	}

	public String getCo_password() {
		return co_password;
	}

	public void setCo_password(String co_password) {
		this.co_password = co_password;
	}

	public String getCo_grade() {
		return co_grade;
	}

	public void setCo_grade(String co_grade) {
		this.co_grade = co_grade;
	}

	public String getCo_reg_date() {
		return co_reg_date;
	}

	public void setCo_reg_date(String co_reg_date) {
		this.co_reg_date = co_reg_date;
	}

	public String getCo_exp_date() {
		return co_exp_date;
	}

	public void setCo_exp_date(String co_exp_date) {
		this.co_exp_date = co_exp_date;
	}

	public String getCo_tags() {
		return co_tags;
	}

	public void setCo_tags(String co_tags) {
		this.co_tags = co_tags;
	}
}
